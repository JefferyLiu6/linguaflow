...
from evals.rag.runner import ROOT, write_new, validate_dataset, report
...
from . import policy as evidence_policy
from evals.rag_v2.runner import identity as base_identity
...
def identity():
    return {**base_identity(),**{str(p.relative_to(ROOT)):fingerprint(p.read_text()) for p in sorted((ROOT/'agent/evals/routing_v5').glob('*.py'))}}


def freeze(dataset,out):
    data=validate_dataset(json.loads(Path(dataset).read_text()))
    plan={'kind':'paired_context_sufficiency_v5','dataset':data,'arms':ARMS,'code':identity(),
        ...
        'limitations':['Frozen synthetic scenarios, AI-authored references; no independent validation.','Common judge scores deterministic sentence/line units, not atomic claims; not directly comparable to the v1 judge.','Live local handler measurements exclude public HTTP/browser hops and judge queue time.','Both arms generate with GPT-4o mini; candidate verification uses GPT-4.1 mini with v4 policy. This paired run combines policy and verifier-model changes.']}
