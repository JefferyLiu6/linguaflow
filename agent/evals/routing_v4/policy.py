"""Offline v4: context sufficiency is distinct from reference coverage.
...
def schema(units):
    properties = {
        ...
        'context_status': {'type':'string','enum':['general_rule','question_complete','card_resolves','missing_referent','not_applicable']},
...
def decode(value, units, question, card):
    ...
    if value['task_kind']=='language' and missing and (not quote or quote not in question):raise ValueError('Missing-reference quote must occur in question')
    if value['task_kind']=='language' and not missing and quote:raise ValueError('Complete question cannot claim a missing reference')
    if value['task_kind']=='language' and value['context_status']=='not_applicable':raise ValueError('Language task requires context category')
    if value['task_kind']=='language' and value['context_status']=='card_resolves' and not any(str(card.get(k) or '').strip() for k in ('instruction','prompt','answer')):
    ...
    return {'decision':decision,'source_id':evidence['source_id'] if evidence else '',
        'support_quote':evidence['text'] if evidence else '', 'rationale':value['task_summary'], 'evidence_reason':value['evidence_reason'],
...
async def verify_evidence(question, card, references):
    ...
    started=time.monotonic(); usage={};attempted=0;error_code='provider_error';raw=None
    try:
        ...
        result=decode(raw,units,question,card)
        ...
        result={'decision':'verification_unavailable','source_id':'','support_quote':'','rationale':'Evidence verification unavailable',
                'error_type':type(exc).__name__,'error_code':error_code,'raw_result':raw}
