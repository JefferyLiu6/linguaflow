...
def identity():
    return {**base_identity(),**{str(p.relative_to(ROOT)):fingerprint(p.read_text()) for p in sorted((ROOT/'agent/evals/routing_v4').glob('*.py'))}}


def freeze(dataset,out):
    data=validate_dataset(json.loads(Path(dataset).read_text()))
    plan={'kind':'paired_context_sufficiency_v4','dataset':data,'arms':ARMS,'code':identity(),
