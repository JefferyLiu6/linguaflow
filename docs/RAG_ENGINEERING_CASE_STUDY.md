...
Development data is for fixing failures. The first 24-case fresh test failed a clarification gate and was retired to regression. After a deterministic context fix and cached routing regression, a new 24-case test evaluates the amended handler under unchanged thresholds. The test construction is adaptive and uses known grammar families. Plans bind code, corpus, labels, snapshots, prices, budgets and targets before requests. Original failures and all attempts stay in Git; private study notes do not. Saved replay reproduces scoring from stored outputs, not stochastic API behavior. The offline instrumentation pins provider timeouts/retries and a 500-token generation cap; production provider wrappers differ. These measurements validate the shared routing/prompt/source behavior under the documented adapter, not exact production latency or failure rates.
