...
Generation uses the existing GPT-4o-mini snapshot. The sequential instrumentation adapter pins embedding timeout to 12 seconds with no retry and generation to 18 seconds/500 tokens with no retry. Production uses its existing provider wrappers and 20-second generation deadline; SDK embedding retry/timeout defaults differ. Routing, prompts, source policy and disclosure are shared, but latency/failure estimates apply to this measured adapter, not a byte-for-byte production provider configuration. Verifier and judge share GPT-4.1 and therefore may share mistakes; record this explicitly. One bounded optional rate-limit retry exists only in the offline judge. A conservative 5,000-token verifier reservation plus the judge's own reservation share a 24,000-token/minute evaluation pacer; waiting occurs outside tutor timing and is reported separately. Report token-based cost estimates, not an invoice or production load benchmark. No private learner history is used or uploaded; cases are public synthetic text.
